...
Ninja project files are generated by [GYP][4] in `out/Debug` and `out/Release`
when you run `gclient sync` or `gclient runhooks`. If you need to re-generate
only the ninja files (like if you've wiped your out folder), run:
...
With [GN][13], Ninja project files are generated by in a directory of your
choice. It can be `out/Debug` or `out/Release` but those are currently occupied
by GYP-generated files (we'll turn that off shortly).

To generate project files using the defaults, run:

~~~~~ bash
gn gen out/Default
~~~~~

See the [GN][13] documentation regarding all available options.

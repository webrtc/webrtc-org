...
**NOTICE:** We're currently migrating from [GYP][4] to [GN][13] for
generating project files. Currently all platforms except iOS are switched to GN
on our bots. With this, only the production code is maintained for a short
period pf time (set `include_tests=0 include_examples=0` in `GYP_DEFINES`).
For Android GN instructions, check
[the Android page]({{ site.baseurl }}/native-code/android/).

##### Using GN

With [GN][13], [Ninja][6] project files are generated by in a directory of your
choice. It can be `out/Debug` or `out/Release` but you can pick any directory
you like.
...
See the [GN][13] documentation for all available options.
